...
CFG = {
    ...
    "kd": 0.01 * np.array([0.3, 0.5, 0.8, 0.1, 0.05, 0.01]), #np.array([0.8, 0.5, 0.3, 0.1, 0.05, 0.01]), # PD 제어 미분 상수
    "max_vel": 0.5, # rad/s
    ...
    # -------- 저장 경로 --------
    "save_dir": "rl_results/SelfTossing_V1_11_TD3"+ "_com" ,#time.strftime("_%Y%m%d_%H%M%S", time.localtime()),
    "actor_save_path": "actor_td3_random_11.pth",
    "critic_save_path": "critic_td3_random_11.pth",
    "results_save_path": "training_results_td3_random.npz",
    "normalizer_save_path": "obs_normalizer_stats_random_11.npz",
...
def main():
    ...
    with open("log.txt", "a") as f:
        f.write(f"실행 시간: {elapsed_time:.6f}초\n")
    print("\n--- 학습 루프 종료. 최종 저장 ---")
    agent.save_models()
    obs_normalizer.save_stats(os.path.join(CFG["save_dir"], CFG["normalizer_save_path"]))
    np.savez(os.path.join(CFG["save_dir"], CFG["results_save_path"]),
