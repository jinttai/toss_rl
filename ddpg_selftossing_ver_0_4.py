...
CFG = {
    ...
    "save_dir": "rl_results/SelfTossing_DenseReward_OrigInit", # 경로명 변경하여 구분
    "actor_save_path": "actor_ddpg_her_dense_originit_v1.pth",
    "critic_save_path": "critic_ddpg_her_dense_originit_v1.pth",
    "results_save_path": "training_results_her_dense_originit_v1.npz"
}
...
class SpaceRobotEnv(gym.Env):
    ...
    def step(self, action):
        des_vel_joints = np.clip(action, -1, 1) * self.cfg["max_vel"]
        ...
        term = False 
        if weld_active:
            if angle_diff_deg <= self.cfg["angle_release_threshold_deg"] or self.step_cnt >= self.horizon -1 :
                if hasattr(self.data, 'eq_active') and len(self.data.eq_active) > 0:
        ...
        if angle_diff_deg <= self.cfg["angle_release_threshold_deg"]:
            reward += 5 # 보상 추가: 릴리즈 성공 시 5점 추가
...
ret_window, success_window = deque(maxlen=100), deque(maxlen=100) # success_window는 이제 새로운 기준으로 채워짐
avg_returns_log, steps_at_avg_log, success_rate_log = [], [], []
actor_loss_log, critic_loss_log = deque(maxlen=100), deque(maxlen=100)
# angle_diff_log는 이제 최종 각도차를 로깅하는데 사용할 수 있음 (또는 평균 유지)
final_angle_diff_log = deque(maxlen=100) # 에피소드 종료 시점 각도차 로깅용
...
try:
    for episode_num in range(CFG['episode_number']):
        ...
        episode_len = 0
        # episode_avg_angle_diff = 0.0 # 평균 대신 마지막 각도차를 주로 볼 것임

        log_qpos_joints, log_qvel_joints, log_des_qvel_joints, log_com_vel_xy = [], [], [], []
        
        last_angle_diff_deg_in_ep = 180.0 # 에피소드 종료 시점의 각도차 저장용
        ...
        for s_idx in range(CFG["episode_length"]):
            ...
            last_angle_diff_deg_in_ep = info_step['angle_diff_deg'] # 매 스텝 각도차 업데이트, 마지막 값이 종료 시점 값
            ...
            if term or trunc: # 에피소드 종료 조건 (term 또는 trunc)
                break 
        ...
        #     for _ in range(CFG["her_replay_k"]):
        #         future_offset = np.random.randint(t_idx, episode_len)
        ...
        ret_window.append(episode_reward_sum) # 총 보상 로깅 (평균 대신)
        final_angle_diff_log.append(last_angle_diff_deg_in_ep)


        if episode_num % 20 == 0:
            avg_total_ret_val = np.mean(ret_window) if ret_window else 0.0 # 이제 평균 총 보상
            ...
            avg_final_angle_val = np.mean(final_angle_diff_log) if final_angle_diff_log else 180.0 # 평균 최종 각도차
            current_success_rate_val = np.mean(success_window) if success_window else 0.0
            
            avg_returns_log.append(avg_total_ret_val) # avg_returns_log는 이제 평균 "총" 보상을 저장
            ...
            print(f"E {episode_num:>5} | L {episode_len:>3} | EpR {episode_reward_sum:>7.2f} | FinalAng {last_angle_diff_deg_in_ep:>6.1f}° | "
                  ...
                  f"Noise {agent.current_noise_std:.3f} | ALoss {avg_a_loss_val:.2e} | CLoss {avg_c_loss_val:.2e} | Steps {total_trained_steps}")
            # --- -------------- ---


        # 1000 에피소드마다 그래프 그리기 (이전과 동일, 파일명만 변경하여 구분 가능)
        if episode_num > 0 and episode_num % 1000 == 0 and episode_len > 0: # 빈도 줄임
            print(f"에피소드 {episode_num}: 데이터 로깅 그래프 생성 중...")
            # ... (그래프 그리는 코드는 이전과 동일하게 유지) ...
            ...
            plt.tight_layout(rect=[0,0,0.85,0.97]); plot_filename = os.path.join(plot_log_dir, f"ep_{episode_num}_final_angle_succ.png")
            plt.savefig(plot_filename); plt.close(fig); print(f"그래프 저장: {plot_filename}")

        
        if episode_num > 0 and episode_num % 500 == 0: # 기존 모델 및 결과 저장
            agent.save_models()
            np.savez(os.path.join(CFG["save_dir"], CFG["results_save_path"]),
                     avg_returns=np.array(avg_returns_log), # 이제 avg_total_returns 임
...
# --- Plotting (메인 학습 그래프) ---
# SUCCESS_THRESHOLD_AVG_EP_REWARD는 이제 이름과 실제 사용 간의 의미가 달라졌으므로,
# 플롯 코드에서 success_rates를 직접 사용하거나, avg_returns의 스케일을 보고 임계값을 다시 설정해야 함.
# 여기서는 success_rate_log에 이미 새로운 기준으로 계산된 성공률이 저장되므로 이를 사용.
os.makedirs(CFG["save_dir"], exist_ok=True)
...
try:
    ...
    avg_returns_log_plot = data['avg_returns'] # 이제 평균 총 에피소드 보상임
    steps_at_avg_log_plot = data['steps_at_avg']
    if 'success_rates' in data: 
        success_rate_log_plot = data['success_rates'] # 새로운 기준으로 계산된 성공률 사용
    else:
        print(f"경고: '{results_file_path}'에 'success_rates' 키 없음.")
        # success_rate_log_plot = np.zeros_like(avg_returns_log_plot) # 임시 처리
...
plot_window_size = 20 
if len(steps_at_avg_log_plot) <= plot_window_size :
...
ax1.set_ylabel(f'Avg Episode Total Reward (Smoothed w={plot_window_size})', color=color_reward) # Y축 레이블 변경
...
ax2.set_ylabel(f'Success Rate (Final Angle <= {CFG["success_angle_threshold_deg"]} deg, Smoothed w={plot_window_size})', color=color_success) # Y축 레이블 변경
...
plt.title(f'DDPG+HER: Dense Reward (Success by Final Angle $\leq$ {CFG["success_angle_threshold_deg"]}$^\circ$)', pad=20) # 그래프 제목 변경
lines1, labels1 = ax1.get_legend_handles_labels()
lines2, labels2 = ax2.get_legend_handles_labels()
if labels1 or labels2: ax2.legend(lines1 + lines2, labels1 + labels2, loc='center right')
fig.tight_layout()
plt.savefig(os.path.join(CFG["save_dir"], "training_performance_plot_final_angle_succ.png")) # 메인 그래프 파일명 변경
