...
CFG = {
    ...
    "actor_save_path": "actor_ddpg_her_angle_v0_3.pth",
    "critic_save_path": "critic_ddpg_her_angle_v0_3.pth",
    "results_save_path": "training_results_her_angle_v0_3.npz"
}
    ...
    def compute_reward(self, com_vel_at_eval_step_3d, desired_angle_goal_scalar, eval_step_count):
        ...
        if eval_step_count == self.goal_timestamp_steps:
            ...
            if achieved_com_vel_xy_norm < 1e-3: # If no significant movement in XY plane
                ...
                aditional_reward = abs(angle_diff)

                if self.similarity > 0.95: # Corresponds to ~18 degrees error
                    reward = 5.0 - aditional_reward # Max reward around 2.0
                elif self.similarity < 0:
                    reward = -1 - aditional_reward
                else:
                    reward = -0.1 - aditional_reward # Penalize larger deviations
        return reward
