...
CFG = {
    ...
    "actor_save_path": "actor_ddpg_her_angle_v1.pth",
    "critic_save_path": "critic_ddpg_her_angle_v1.pth",
    "results_save_path": "training_results_her_angle_v1.npz"
}
...
class SpaceRobotEnv(gym.Env):
    ...
    def step(self, action): # action is 6D for arm joints
        ...
        # Reward is calculated based on state at goal_timestamp_steps, so 0 here for intermediate steps
        reward = 0.0 
    ...
    def _calculate_com(self): # Calculates 3D COM position and velocity
        ...
        com_vel = np.zeros(3)
        total_mass = 0.0
        # Iterate through relevant body IDs (e.g., 1 to model.nbody-1, or specific named bodies)
        # Assuming bodies 1 through 9 are relevant as in original snippet
        ...
        if eval_step_count == self.goal_timestamp_steps:
            ...
            if achieved_com_vel_xy_norm < 1e-3: # If no significant movement in XY plane
                ...
                if self.similarity > 0.95: # Corresponds to ~18 degrees error
                    reward = 1.0 + self.similarity # Max reward around 2.0
    ...
    def _rotate_vector_by_quaternion(self, vector, quat_rotation_wxyz): # Assumes quat is [w,x,y,z]
        # Scipy Rotation for robust vector rotation
        # Convert quat_rotation_wxyz [w,x,y,z] to [x,y,z,w] for Scipy
...
try:
    for episode_num in range(CFG['episode_number']):
        ...
        for ro_t, a_t, rno_t, term_t, trunc_t, info_s_t_plus_1 in episode_transitions_buffer:
            ...
            if achieved_com_vel_3d_at_goal_ts is not None:
                achieved_xy_norm = np.linalg.norm(achieved_com_vel_3d_at_goal_ts[:2])
                if achieved_xy_norm > 1e-3: # Only use if there was significant XY motion for hindsight goal
                    hindsight_achieved_angle = np.arctan2(achieved_com_vel_3d_at_goal_ts[1], achieved_com_vel_3d_at_goal_ts[0])
                    hindsight_goal_angle_array = np.array([hindsight_achieved_angle])

                    if np.random.rand() < (CFG["her_replay_k"] / (1 + CFG["her_replay_k"])):
                        reward_hindsight = env.compute_reward(
                            info_s_t_plus_1['com_vel_3d'],
                            hindsight_goal_angle_array[0], # scalar hindsight angle
                            info_s_t_plus_1['step_count_in_episode']
                        )
                        buf.add(ro_t, a_t, reward_hindsight, rno_t, done_flag, hindsight_goal_angle_array)
