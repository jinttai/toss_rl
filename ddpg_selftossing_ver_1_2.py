...
CFG = {
    ...
    "save_dir": "rl_results/SelfTossing_DenseReward_HER_Velocity_Reward", # 경로명 변경하여 구분
    "actor_save_path": "actor_ddpg_her_dense_originit_v1_2.pth",
    "critic_save_path": "critic_ddpg_her_dense_originit_v1_2.pth",
    "results_save_path": "training_results_her_dense_originit_v1_2.npz"
}
...
class SpaceRobotEnv(gym.Env):
    ...
    def step(self, action):
        des_vel_joints = np.clip(action, -1, 1) * self.cfg["max_torque"]
        ...
        term = False 
        if weld_active:
            if (angle_diff_deg <= self.cfg["angle_release_threshold_deg"] and velocity > 0.1) or self.step_cnt >= self.horizon -1 :
                if hasattr(self.data, 'eq_active') and len(self.data.eq_active) > 0:
    ...
    def compute_reward(self, com_vel_3d_at_this_step, desired_angle_goal_scalar):
        ...
        velocity_reward = (np.linalg.norm(com_vel_3d_at_this_step[:2]) - 0.1) ** 2 * self.cfg["velocity_reward_weight"]
        reward =  - abs(angle_diff_rad) **  + velocity_reward
