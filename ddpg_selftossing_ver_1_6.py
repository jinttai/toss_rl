...
CFG = {
    # -------- mujoco --------
    "model_path": "mujoco_src/spacerobot_twoarm_3dof_ee_fixed.xml",
    "model_path_fixed": 'mujoco_src/spacerobot_twoarm_3dof_base_fixed.xml',

    # -------- PD 제어 --------
    "kp": 10.0 * np.array([1, 1, 1, 0.1, 0.05, 0.01]), # PD 제어 비례 상수
    "kd": 0,
    "max_vel": 0.5, # rad/s
    ...
    # -------- 저장 경로 --------
    "save_dir": "rl_results/SelfTossing_TD3_RandomGoal",
    "actor_save_path": "actor_td3_random.pth",
    "critic_save_path": "critic_td3_random.pth",
    "results_save_path": "training_results_td3_random.npz",
    "normalizer_save_path": "obs_normalizer_stats_random.npz",
...
class SpaceRobotEnv(gym.Env):
    ...
    def _raw_obs(self):
        qpos_joints = self.data.qpos.copy()
        qvel_joints = self.data.qvel.copy()
    ...
    def _apply_pd_control(self, des_velocity):
        current_joint_velocity = self.data.qvel
        qacc = self.data.qacc
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.data.qpos= rng.uniform(-np.pi, np.pi, 6)
        self.data.qvel = np.zeros_like(self.data.qvel)
        mujoco.mj_forward(self.model, self.data)
        
        self.last_action = np.zeros(self.action_space.shape[0])
        
        ### CHANGED ###: 목표 각도를 매 에피소드마다 무작위로 설정
        random_target_angle = rng.uniform(-np.pi, np.pi)
        self.current_episode_goal = np.array([
            random_target_angle,
            self.cfg["target_velocity_magnitude"]
        ])
        # step 함수에서 사용되는 목표 각도도 업데이트
    ...
    def compute_reward(self, time_step, com_vel_3d, desired_goal, action, last_action):
        ...
        reward_angle = -(angle_err**2 + np.log10(1e-6 + np.abs(angle_err)))
        reward_angle *= w_t
        

        # 행동 변화율에 대한 패널티
        action_rate_penalty = -self.cfg["action_rate_penalty_weight"] * np.mean((action - last_action)**2)
        reward = reward_angle + action_rate_penalty
        if time_step >= self.horizon - 1:
...
def main():
    ...
    for episode_num in range(CFG['episode_number']):
        ...
        # Hindsight Experience Replay (HER)를 위한 가상 경험 추가
        for t_idx, (ro_t, a_t, _, rno_t, term_t, trunc_t, info_s_t_plus_1) in enumerate(episode_transitions_buffer):
            ...
            # HER을 위한 이전 행동 가져오기
            last_action_her = episode_transitions_buffer[t_idx-1][1] if t_idx > 0 else np.zeros_like(a_t)
            
            # HER 보상 재계산
            total_her_reward = env.compute_reward(
                t_idx, 
                info_s_t_plus_1['com_vel_3d'], 
                her_new_desired_goal, 
                a_t,               # 현재 행동
                last_action_her    # 이전 행동
            )
            buf.add(ro_t, a_t, total_her_reward, rno_t, float(term_t or trunc_t), her_new_desired_goal)
